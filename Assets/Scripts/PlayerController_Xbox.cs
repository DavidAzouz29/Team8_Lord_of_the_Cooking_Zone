...
public class PlayerController_Xbox : MonoBehaviour {
	...
	void FixedUpdate () 
	{
		...
		Vector3 targetVelocity = new Vector3(CheckMoveDirection(Input.GetAxis("Horizontal_" + gameObject.name),transform.right) * strafeSpeed, 0,CheckMoveDirection(Input.GetAxis ("Vertical_" + gameObject.name),transform.forward)* forwardAndBackSpeed);
	...
	private void MoveHead()
	{
		...
		{
			float rotationX = transform.localEulerAngles.y + Input.GetAxis("Mouse X_" + gameObject.name) * sensitivityX;

			rotationY += Input.GetAxis("Mouse Y_" + gameObject.name) * sensitivityY;
		...
		{
			transform.Rotate(0, Input.GetAxis("Mouse X_" + gameObject.name) * sensitivityX, 0);
		}
		else
		{
			rotationY += Input.GetAxis("Mouse Y_" + gameObject.name) * sensitivityY;
	...
	private void MoveBody()
	{
        ...
        if (isGrounded && Input.GetButton("Sprint_" + gameObject.name))
        {
            isRunning = true;
        }
        else
		...
		//if (Input.GetButtonUp(KeyCode.LeftShift)) {
			//
		//}
	}
